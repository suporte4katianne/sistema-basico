...
public class NotaFiscalBean implements Serializable {
	...
	public void listarNotasFiscais(){
		if(facesContext.getViewRoot().getViewId().contains("Saida")){
			notasFiscais = gestaoNotaFiscal.listarNotasFiscais("1");
		} else {
			notasFiscais = gestaoNotaFiscal.listarNotasFiscais("0");
		}
	}
	...
	public void transmitirNfe() throws Exception {
		...
		if (notaFiscal != null) {
			if (notaFiscal.getMensagemRetorno().contains("Autorizado")) {
				...
				gerarDanfe();
				listaDeDestinatarios = notaFiscal.getEmailEmpresa() + ";" + notaFiscal.getEmailEntidade();
				enviarNotaPorEmail();
			} else {
